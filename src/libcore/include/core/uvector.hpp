...
/**
 * @brief A container similar to std::vector, but one that allows
 * construction without initializing its elements.
 * @details This container is similar to a std::vector, except that it can
 * be constructor without
 * initializing its elements. This saves the overhead of initialization,
 * hence the
 * constructor @ref uvector(size_t) is significantly faster than the
 * corresponding std::vector
 * constructor, and has no overhead to a manually allocated array.
 *
 * Probably its greatest strength lies in the construction of containers
 ...
 *
 * The container will behave correctly with any trivial type, but will not
 * work for almost
 * all non-trivial types.
 *
 * The element type must be trivial. Because of the use of @c memcpy and @c
 * memmove,
 * the @ref push_back() and @ref insert() methods are a bit faster than the
 * std::vector
 ...
 *
 * All other members work exactly like std::vector's members, although some
 * are slightly faster because of
 ...
 *
 * @author André Offringa
 * @copyright André Offringa, 2013, distributed under the GPL license
 * version 3.
 */
...
class uvector : private Alloc {
  ...
  public:
     ...
     * @param allocator Allocator used for allocating and deallocating
     * memory.
     */
     ...
     * initializing these.
     * @details This constructor deviates from std::vector's behaviour,
     * because it will not
     * value construct its elements. It is therefore faster than the
     * corresponding constructor
     * of std::vector.
     * @param n Number of elements that the uvector will be initialized
     * with.
     */
    explicit uvector(size_t n) : _begin(allocate(n)), _end(_begin + n), _endOfStorage(_end) {}

    /** @brief Construct a vector with given amount of elements and set
     * these to a specific value.
     * @details This constructor will initialize its members with the given
     * value.
     * @param n Number of elements that the uvector will be initialized
     * with.
    ...
    uvector(InputIterator first, InputIterator last, const allocator_type &allocator = Alloc()) : Alloc(allocator) {
    ...
    uvector(const uvector<Tp, Alloc> &other)
      : Alloc(std::allocator_traits<Alloc>::select_on_container_copy_construction(static_cast<allocator_type>(other))),
        _begin(allocate(other.size())), _end(_begin + other.size()), _endOfStorage(_end) {
     ...
     * @param initlist Initializer list used for initializing the new
     * uvector.
     * @param allocator Allocator used for allocating and deallocating
     * memory.
     */
     ...
     * @details The allocator of the uvector will be assigned to @p other
     * when
    ...
    /** @brief Get constant reverse iterator to element before first
     * element. */
     ...
     * @details If the new size is larger than the current size, new values
     * will be
     * left uninitialized. Therefore, it is more efficient than @c
     * resize(size_t) in
     * @c std::vector, as well as @ref resize(size_t, const Tp&).
     * If the new size is smaller than the current size, the container will
     * be
     * truncated and elements past the new size will be removed. No
     * destructor of the
    ...
    void resize(size_t n, const Tp &val) {
        ...
        _end = _begin + n;
        if (oldSize < n) std::uninitialized_fill<Tp *, Tp>(_begin + oldSize, _end, val);
    }

    /** @brief Get the number of elements the container can currently hold
    ...
    /** @brief Reserve space for a number of elements, to prevent the
     * overhead of extra
     * reallocations.
     * @details This has no effect on the working of the uvector, except
     * that it might change
     * the current capacity. This can enhance performance when a large
     * number of elements are added,
    ...
    /** @brief Change the capacity of the container such that no extra space
     * is hold.
     * @details This has no effect on the working of the uvector, except
     * that it might change
     * the current capacity. This can reduce the current memory usage of the
     * container.
     *
     * This method might cause a reallocation, causing iterators to be
     * invalidated.
    ...
    /** @brief Resize the container and assign the given value to all
     * elements.
    ...
    /** @brief Add the given value to the end of the container by moving it
     * in.
     * @details Iterators are invalidated.
     *
     * Note that this container can only hold simple types that do not
     * perform allocations. Therefore,
     * there is probably no benefit in moving the new item in over copying
     * it in with @ref push_back(const Tp&).
     * @param item Value of new element.
     */
     ...
     * @details All iterators will be invalidated. This operation needs to
     * move all elements after
     * the new element, and can therefore be expensive.
     * @param position Position of the new element. The new element will be
     * added before the old element
     * at that position.
    ...
    /** @brief Insert elements at a given position and initialize them with
     * a value.
     * @details All iterators will be invalidated. This operation needs to
     * move all elements after
     * the new element, and can therefore be expensive.
     * @param position Position of the new elements. The new elements will
     * be added before the old element
     * at that position.
    ...
    /** @brief Insert elements at a given position and initialize them from
     * a range.
     * @details All iterators will be invalidated. This operation needs to
     * move all elements after
     * the new element, and can therefore be expensive.
     ...
     * @details This operation moves all elements past the removed element,
     * and can therefore be
     * expensive.
     * @param position Position of element to be removed.
     * @return Iterator pointing to the first element past the delete
     * element.
     */
     ...
     * @details This operation moves all elements past the removed elements,
     * and can therefore be
     * expensive.
     ...
     * @details Iterators to both vectors will remain valid and will point
     * into
     * to the swapped container afterwards. This function will never
     * reallocate
     * space.
     *
     * The allocator will be swapped when the @c propagate_on_container_swap
     * of the respective @c allocator_trait is @c true_type.
     * Its behaviour is undefined when the allocators do not compare equal
     * and
    ...
    /** @brief Insert an element at a given position by constructing it in
     * place.
     * @details All iterators will be invalidated. This operation needs to
     * move all elements after
     * the new element, and can therefore be expensive.
     ...
     * added before the old element
     * at that position.
     * @param args List of arguments to be forwarded to construct the new
     * element.
     * @return Position of the new element.
     */
    ...
    /** @brief Add the given value to the end of the container by
    ...
    /** @brief Insert elements at a given position without initializing
     * them.
     * @details All iterators will be invalidated. This operation needs to
     * move all elements after
     * the new element, and can therefore be expensive. It will not
     * initialize the new elements,
     * and is therefore faster than @ref insert(const_iterator, size_t,
     * const Tp&).
     *
    ...
    /** @brief Add elements from an initializer list to the end of the
     * container.
     * @details All iterators will be invalidated.
     *
     * This method is non-standard: it is not present in std::vector.
  ...
  private:
    ...
    void deallocate() { deallocate(_begin, capacity()); }
    ...
    // This function is called from assign(iter,iter) when Tp is an
    // integral. In that case,
    // the user tried to call assign(n, &val), but it got caught by the
    // wrong overload.
    template <typename Integral>
    ...
    void check_bounds(size_t index) const {
        if (index >= size()) throw std::out_of_range("Access to element in uvector past end");
    }
    ...
    // implementation of operator=() with
    // propagate_on_container_copy_assignment
    ...
    uvector &assign_move_from(uvector<Tp, Alloc> &&other, std::false_type) {
        if (static_cast<Alloc &>(other) == static_cast<Alloc &>(*this)) {
            ...
            // We should not propagate the allocator and the allocators are
            // different.
            // This means we can not swap the allocated space, since then we
            // would
            // deallocate the space with a different allocator type.
            // Therefore, we
            // need to copy:
            assign_copy_from(other, std::false_type());
        }
    ...
    void swap(uvector<Tp, Alloc> &other, std::false_type) {
         ...
         * - Do not swap the allocators. For stateful allocators, we would
         * need to
         *   reallocate memory, and iterators would not be valid UNLESS
         *   they were stored as indices. However, containers with stateful
         * allocators
         *   are not allowed to be swapped unless the allocators are equal,
         * in which case swapping
         *   is not necessary.
         ...
         *
         * The standard says:
         * "Allocator replacement is performed by copy assignment, move
         * assignment, or
         * swapping of the allocator only if
         * allocator_traits<allocatortype>::
         * propagate_on_container_copy_assignment::value,
         * allocator_traits<allocatortype>::propagate_on_container_move_assignment::value,
         * or
         * allocator_traits<allocatortype>::propagate_on_container_swap::value
         * is true
         * within the implementation of the corresponding container
         * operation. The behavior
         * of a call to a container’s swap function is undefined unless the
         * objects being
    ...
    // This function is called from push_back(iter,iter) when Tp is an
    // integral. In that case,
    // the user tried to call push_back(n, &val), but it got caught by the
    // wrong overload.
    template <typename Integral>
 ...
 * @details If two uvectors compare equal up to the length of one, the
 * uvector with
